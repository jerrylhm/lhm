...
<body>
			...
			<div class="container-table">
				<div>
					<table class="table table-striped table-hover" id="table">
						...
						<tbody>
							<c:forEach items="${list_bck }" var="bck" varStatus="stas">
								<tr id="tr_${group.urg_id}">
									<td>
										<input class="check-user" type="checkbox" value="${bck.bck_mm_id}" style="cursor: pointer">
									</td>
									<td title="${stas.count }">
										${stas.count }
									</td>
									<td title="${bck.bck_mm_name}">
										${bck.bck_mm_name}
									</td>
									<td title="${bck.node_name}">
										${bck.node_name}
									</td>
									<c:if test="${bck.bck_mm_state==1}">
										<td title="已启用">
											已启用
										</td>
									</c:if>
									<c:if test="${bck.bck_mm_state==0}">
										<td title="已禁用">
											已禁用
										</td>
									</c:if>
									...
									<td style="text-align: left">
										<a class="btn btn-success btn-sm" title="编辑" onclick="showUpdateModal('${bck.bck_mm_id }','${bck.bck_mm_name}','${bck.bck_mm_treeid}')">
											<i class="fa fa-pencil fa-lg fa-fw"></i>编辑
										</a>
										<a class="btn btn-success btn-sm" title="启用/禁用" onclick="onOroff('${bck.bck_mm_id }','${bck.bck_mm_name}','${bck.bck_mm_treeid}')">
											<i class="fa fa-pencil fa-lg fa-fw"></i>启用/禁用
										</a>
										<a class="btn btn-danger btn-sm btn-margin" title="删除" onclick="deleteSingle('${bck.bck_mm_id }')">
					...
					<div class="modal-dialog">
						<div class="modal-content">
							<div class="modal-header">
								...
								<h5 class="modal-title">新增后台菜单</h5>
							</div>
								...
								<div class="modal-container">
									<form class="form-horizontal">
										<div class="form-group">
											...
											<div class="col-sm-8">
												<input type="text" id="input_Groupname" class="form-control" placeholder="输入菜单名">
											</div>
										</div>
										...
										<div class="form-group">
											...
											<div class="col-sm-8">
												<select class="form-control" id="input_Grouptree">
													<c:forEach items="${list_tree}" var="tree">
	...
	<script type="text/javascript">
		...
		function addUserGroup(){
			...
			var Group_data = {
					"bck_mm_name":name,
					"bck_mm_treeid":treeid
			};
			
			$.post("<%=basePath %>backgroundMenuManagement/addMenu",Group_data,function(msg){
				if(msg == "1"){
					$("#addModal").modal("hide");
					alertSuccess("菜单新增成功！");
					setTimeout(function(){
		...
		function first() {
			...
			//跳转
			window.location.href = "<%=basePath%>backgroundMenuManagement/index";
		}
		...
		function pre() {
			...
			//跳转
			index--;
			window.location.href = "<%=basePath%>backgroundMenuManagement/index?page="+ index;
		}
		...
		function last() {
			...
			//跳转
			window.location.href = "<%=basePath%>backgroundMenuManagement/index?page=${countPage}";
		}
		...
		function gotoPage() {
			...
			//跳转
			window.location.href = "<%=basePath%>backgroundMenuManagement/index?page="+ parseInt(page);
		}
		...
		function deleteChecked(){
			...
			if(Num == 0){
				alertFail("未选中菜单；无法删除！");
				return false;
			}
			deleteUserGroup("确定删除选中的"+Num+"条菜单？",idAll);
		}
		
		//单个删除
		function deleteSingle(id){
			deleteUserGroup("确定删除该条菜单？",id);
		}
		...
		function deleteUserGroup(title,idAll){
			...
			swal({
				...
				if(isConfirm) {
					$.ajax({
						type:"post",
						url:"<%=basePath %>backgroundMenuManagement/deleteMenu",
						data:{"ids":idAll},
		...
		function updateUserGroup(){
			...
			$.ajax({
				type:"post",
				url:"<%=basePath%>backgroundMenuManagement/updateMenu",
				data:{"bck_mm_id":id,"bck_mm_name":name,"bck_mm_treeid":tree},
				success:function(msg){
					if(msg == "1"){
						alertSuccess("修改成功！");
						setTimeout(function(){
		...
		//启用/禁用菜单
		function onOroff(id,name,treeid){
			$.ajax({
				type:"post",
				url:"<%=basePath%>backgroundMenuManagement/updateMenu",
				data:{"bck_mm_id":id,"bck_mm_name":name,"bck_mm_treeid":treeid,"change_state":"1"},
				success:function(msg){
					if(msg == "1"){
						alertSuccess("启动成功！");
						setTimeout(function(){
