...
/*  useritem  */ 
.draggable {
   width: 12px;background-color:#00bcd4;position: absolute;
}
...
.useritem-title {
   border: none;outline: none;width: auto;width: 10px;min-width: 10px;font-size: 20px;
}
.editeitem {
   z-index:50; 
}
...
<div class="draggable" >
  <div class="useritem-content">
     ...
     <label class="useritem-title" style="display: none;width: auto;">当前温度</label><input class="tree-user-item useritem-title" type="text" value="当前温度"><span class="tree-user-item useritem-content" style="color: red;font-size: 15px">25℃</span>
  </div>  
</div>
</div>
<!-- button模板 -->
<div class="model button-model">
<div class="draggable">
  <div class="useritem-content">
     ...
     <div class="useritem-titlespace">
     <label class="useritem-buttontitle">开关</label>
     </div>
  </div>
</div>
</div>
...
$(function(){
	var userHandleJsonStr = '${userHandleJson}';
	var userHandleJson;
	var valueJsonStr = '${valueJson}';
	var valueJson;
    if(userHandleJsonStr == '') {
    	
    }else {
    	userHandleJson = JSON.parse(userHandleJsonStr);
    }
    ...
    if(valueJsonStr == '') {
    	...
    	var valuesLength = valueJson.length;
    	for(var i = 0; i < valuesLength; i++) {
    		var valueType = valueJson[i].value_type;
    		//如果组件类型时按钮，则在操作界面添加一个按钮
    		if(valueType == 'button') {
    			var $button = $('.button-model').clone(true);
    			$button.removeClass('button-model');
    			$button.removeClass('model');
    			$button.find('.useritem-buttontitle').html(valueJson[i].value_key);
    			$('body').append($button);
    		//如果组件类型是文本，则在操作界面添加一条文本
    		}else if(valueType == 'text') {
    			var $text = $('.text-model').clone(true);
    			$text.removeClass('text-model');
    			$text.removeClass('model');
    			$text.find('input.useritem-title').val(valueJson[i].value_key);
    			$text.find('span.useritem-content').html(valueJson[i].value_data);
    			$('body').append($text);
    		}
    	}
    }
...
$(function(){
	...
	$(".resizable").resizable({resize:function(event, ui){
		...
		$(obj).next('.useritem-titlespace').css('width', $(obj).css('width'));
		$(obj).next('.useritem-titlespace').children().css('width', $(obj).css('width'));
	}});
...
$(function() {
	...
	$('.useritem-buttontitle').each(function(){
		$(this).parent().css('width', $(this).parent().prev('div').css('width'));
		$(this).css('width', $(this).parent().prev('div').css('width'));
	});
})
